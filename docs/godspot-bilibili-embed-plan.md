...
5. iframe 预览和卡片展示。
6. 回归测试和正式发版。
7. B 站外链视频建议一定要保存 coverUrl（封面图）。不然列表里全是黑框或统一图标，看起来不够直观。
...
最后补充
 两个需要注意的“坑”
虽然标签好写，但 B 站的嵌入有两个小限制需要避开：

点击跳转问题：B 站默认的 iframe 可能会在点击时跳转到 B 站官网。我们需要在 src 链接后面加上特定的参数（如 high_quality=1）来优化清晰度，或者通过 CSS 禁用一些不必要的点击层。


高度自适应：视频的比例通常是 16:9。使用 Tailwind 的 aspect-video 类可以完美解决在手机端和电脑端变形的问题
...
- **调整神人点位上传布局**：上传视频小面板移动到左侧，右侧集中展示地图筛选、视频预览和视频列表
- **新增 B 站链接标题识别**：上传面板支持粘贴 B 站 / b23.tv 链接自动识别视频标题并默认填入，仍可手动自定义标题

v1.3.3   日期2026.5.11---1
说明
- **修复"取消编辑后修改仍保留"**：编辑模式进入时会快照标题/副标题/界面自定义等本地 UI 偏好，点击"取消"后整套设置会回滚到编辑前的状态，仅"保存"才真正保留
- **新增枪械支持自定义"配置方案"**：新增枪械弹窗与卡片内编辑均改为可手动输入的文本框，附带常用方案下拉建议（满改 / 半改 / 丐版 / 平民改 等），不再被固定四项限制
- **新增枪械弹窗整体改版**：标题增加渐变头部卡片，图鉴分类改为芯片式选择，评级改为彩色分段切换，并新增独立底部操作栏，整体更接近主流表单 UI
- **界面自定义面板瘦身可折叠**：默认收起为单行密度预设条，点击"更多"可展开高级设置；列数、每页卡片数、每张卡片配置条数等数值改为可加减/直接输入，支持 8、9、10、11、12 等任意整数
- **卡片网格支持任意列数与间距**：移除 Tailwind 固定 `gap-*` / `grid-cols-*` 映射，改为内联媒体查询 CSS 注入，列数（1–8）与间距（0–48px）均可自由设定且立刻生效
...
v1.4.1   日期2026.5.16---2
说明
- **管理面板 UI 改造**：参考首页突击步枪/战斗步枪/冲锋枪等横向按钮风格，将管理面板顶部改为"概览 / 邀请码 / 用户管理 / 监控面板"四段切换条，移动端可横向滚动
- **概览页**：合并展示用户、邀请码、访问数四组核心指标卡，并显示最近 5 条访问记录
- **新增监控面板**：含访问统计四卡（总访问 / 24h / 1h / 24h 独立 IP）和访问日志表格（时间、IP / 地区、页面路径、来源、UA / 已登录用户名），支持手动刷新和一键清空
- **服务端访问日志**：新增 `accessLogMiddleware`，仅记录页面级 GET（自动跳过 `/api/`、静态资源、`/uploads`、`/godspot-files`），落盘 `runtime/access_log.json` 循环 buffer 最多 5000 条，每分钟批量刷新避免 IO 峰值
- **IP 地区识别**：解析 Cloudflare 提供的 `cf-ipcountry` `cf-region` `cf-ipcity` 头，未通过 CF 时降级为"未知地区"
- **隐私与稳健**：UA 截断 250 字符，path/referer 截断 200 字符，避免日志被超长 URL 撑爆；写入失败仅记 warn 不影响主流程
- **新增 `/api/admin/access-logs`**：GET 拉取最近日志（默认 500 条上限），DELETE 清空，仅管理员可访问

v1.4.2   日期2026.5.16---3
说明
- **修复邀请码注册始终报"请输入邀请码"**：`App.tsx` 的 `onRegister` 包装函数仅接收 `(u, p)` 两个参数，导致前端组件传给它的 `inviteCode` 被丢弃，最终请求体里没有邀请码字段；修复为透传第三个参数 `code`
- **AuthModal 文案精简**：移除"首位管理员可留空"提示，避免误导（首位管理员是后端逻辑，前端无需暴露）
- **真实 IP 与地区识别**：
  - `app.set("trust proxy", true)`：在 Cloudflare → OpenResty → Node 多跳反代下也能正确还原客户端真实 IP

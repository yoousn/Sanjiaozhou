...
export function AddGunModal({ isOpen, onClose, onConfirm }: { isOpen: boolean, onClose: () => void, onConfirm: (name: string, category: string, variant: Omit<GunVariant, 'id'>) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div
        className="w-full max-w-md bg-white rounded-3xl shadow-2xl p-6 md:p-8 relative z-10 border border-white/20 transition-all"
      >
        <button type="button" onClick={onClose} className="absolute right-6 top-6 text-zinc-400 hover:text-zinc-900 transition-colors bg-zinc-100 hover:bg-zinc-200 rounded-full p-2 outline-none focus:ring-2 focus:ring-zinc-900/20">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          <div className="grid grid-cols-2 gap-4">
             <div>
              ...
              <input required autoFocus className={cn(inputClasses, "py-3 bg-white border border-zinc-200 shadow-sm text-sm")} placeholder="e.g. M4A1..." value={name} onChange={r => setName(r.target.value)} />
            </div>
            <div>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <select className={cn(inputClasses, "py-3 bg-white border border-zinc-200 shadow-sm text-sm")} value={tier} onChange={r => setTier(r.target.value)}>
            ...
            <div>
              ...
              <input required className={cn(inputClasses, "py-3 bg-white border border-zinc-200 shadow-sm text-sm")} placeholder="15W" value={price} onChange={r => setPrice(r.target.value)} />
            </div>
            <div>
              <label className="block text-xs font-bold text-zinc-500 mb-2 uppercase tracking-widest">配置方案</label>
              <select className={cn(inputClasses, "py-3 bg-white border border-zinc-200 shadow-sm text-sm")} value={buildType} onChange={r => setBuildType(r.target.value)}>
          ...
          <div>
            ...
            <input required className={cn(inputClasses, "py-3 font-mono bg-white border border-zinc-200 shadow-sm text-sm")} placeholder="Delta-XXX..." value={code} onChange={r => setCode(r.target.value)} />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-zinc-500 mb-2 uppercase tracking-widest">来源作者</label>
              <input className={cn(inputClasses, "py-3 bg-white border border-zinc-200 shadow-sm text-sm")} placeholder="UP主名" value={author} onChange={r => setAuthor(r.target.value)} />
            </div>
            <div>
              <label className="block text-xs font-bold text-zinc-500 mb-2 uppercase tracking-widest">来源链接</label>
              <input className={cn(inputClasses, "py-3 bg-white border border-zinc-200 shadow-sm text-sm")} placeholder="https://..." value={sourceUrl} onChange={r => setSourceUrl(r.target.value)} />
            </div>
          </div>

          <button type="submit" className="mt-4 w-full py-4 rounded-2xl bg-zinc-900 text-white font-black tracking-widest uppercase hover:bg-zinc-800 hover:scale-[1.02] transition-all focus:ring-2 focus:ring-zinc-900/20 focus:outline-none shadow-xl shadow-zinc-900/10">
            完成添加
          </button>
        </form>
      </div>
